...
data CTExpr = CTApp CTExpr CTExpr                 -- generic application
            | RecApp CTExpr CTExpr                -- application of a fixpoint
            ...
            -- plus the ambiguous 'resume' for the demo compiler (2010.12.28)
            | Resume CTExpr CTExpr

            | Lkp CTIdent
            | XEnv CTExpr CTExpr CTExpr
            ...
            -- control flow:
            | CTBranch CTExpr CTExpr CTExpr        -- 'if-then-else'
            | CTCase CTExpr [(CTPat, CTExpr)]      -- case expression
            | CaseStar CTExpr [(CTPat, CTExpr)]    -- resumptive case 
...
instance PPT CTExpr where
  ...
  ppt RdEnv             = "rdEnv"
  ppt (InEnv rho t)     = "inEnv " ++ (ppt rho) ++ ' ':(ppt t)
  ppt (Resume r v)      = "resume " ++ (ppt r) ++ ' ':(ppt v)

  ppt (XEnv x y z)      = "xEnv " ++ (ppt x) ++ ' ':(ppt y) ++ ' ':(ppt z)
  ppt (Lkp x)           = "lkp " ++ x
  ...
  ppt (CaseStar e as) = "case* " ++ '(':(ppt e) ++ ") of\n" ++
                        (foldr (\(p, e) -> \s -> '\t':(ppt p) ++ " -> "
                                                 ++ (ppt e)
                                                 ++ ('\n':s)) "" as)
